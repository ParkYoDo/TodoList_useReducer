...
function TodoModifyUserModal({ show, setShow }) {
  ...
  const modifyUser = (e) => {
    ...
    if (!input) {
      ...
      show.name === 'name' && dispatch({ type: 'TODO_MODIFY', writer: input });
      dispatch({ type: 'USER_MODIFY', key: show.name, value: input });
      dispatch({ type: 'LOGINUSER_MODIFY', key: show.name, value: input });
      closeModal();
    }
  };
